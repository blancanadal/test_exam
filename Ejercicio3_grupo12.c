...
static void MessageFunction(void *event_data) {
    ...
    // TODO: ver enunciado
    if (strstr(data_len, "delayed")|| strstr(data_len,"completed")){
    machines_generating_notifications->machine_name[num_machines_generating_notifications] = p_machine_name;
    num_machines_generating_notifications = num_machines_generating_notifications + 1;
    }
    else {
        NULL;
    }
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        //msg_id = esp_mqtt_client_subscribe(client, "LSE/machines/+/worker_assignment", 0);
        msg_id = esp_mqtt_client_subscribe(client, "LSE/machines/+/progress", 0);
        ...
        if(MQTT_CONNECTED)
        {
            for(int i=0; i < num_machines_generating_notifications; i++)
            {
                ...
                if(strlen(machines_generating_notifications[i].worker_assignment)>0) {
                    ...
                    // TO DO
                    if (strstr(machines_generating_notifications[i].progress, "completed"))
                    {
                        msg_id = esp_mqtt_client_publish(mqtt_client, "LSE/blanca.nadal@alumnos.upm.es/workers/+/worker_performance", "good", 0, 1, 0);
                        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
                    }

                    else if (strstr(machines_generating_notifications[i].progress, "delayed"))
                    {
                        msg_id = esp_mqtt_client_publish(mqtt_client, "LSE/blanca.nadal@alumnos.upm.es/workers/+/worker_performance", "bad", 0, 1, 0);
                        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
                    }
                    else {
                        NULL;
