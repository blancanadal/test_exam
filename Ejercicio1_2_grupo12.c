...
machine_t student_machine = {
    .machine_name = "machine23",
    .IP_address = "10.8.42.23",
    .current_status = "running",
    .work_assignment = "testing",
    .worker_assignment = "blanca.nadal@alumnos.upm.es",
    .progress = "normal",
...
{
    char *pch;
    pch = strtok(topic_string, "/");      // IGUAL A SPLIT EN PY 
    pch = strtok(NULL, "/");
    pch = strtok(NULL, "/");              // LO HACE TRES VECES PARA LLEGAR A MACHINE12
    return pch;
}
...
int find_machine_index(char *machine_name, int num_machines_generating_notifications)
{
    ...
    if (num_machines_generating_notifications > 0) {
        ...
        {
            if (strcmp(machines_generating_notifications[i].machine_name, machine_name) == 0)   // STRCMP compara dos strings y devuelve 0 si son iguales
            {
                //printf("\r\nFOUND in position %d %s\r\n", i, machine_name);
                index = i;
                break;
            }
        }    
    ...
    return index;
    // VERIFICA SI MI MÁQUINA ESTÁ GENERANDO NOTIFICACIONES
}
...
static void MessageFunction(void *event_data) {
    ...
    strcpy(machine_name, p_machine_name);   // COPIA EL STRING QUE ACABA DE EXTRAER EN MACHINE_NAME
    printf("machine_name = %s\r\n", machine_name);
    //printf("topic = %s\r\n", topic);
    ESP_LOGI(TAG, "machine_name = %.*s\r\n", strlen(machine_name), machine_name);

    // TODO: ver enunciado
    if (strstr(data_len, "idle")){  // Changes the state of current_state to the opposite
      msg_id = esp_mqtt_client_publish(mqtt_client, "LSE/machines/+/current_status", "running", 0, 1, 0);
      ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
    }
    else {
      msg_id = esp_mqtt_client_publish(mqtt_client, "LSE/machines/+/current_status", "idle", 0, 1, 0);
      ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
    }
    // TODO: ver enunciado
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        //msg_id = esp_mqtt_client_subscribe(client, "LSE/machines/+/worker_assignment", 0); -- From here we get machine12
        msg_id = esp_mqtt_client_subscribe(client, "LSE/machines/+/current_status", 0);
...
void Publisher_Task(void *params)null
{
    int msg_id = 0;
    ESP_LOGI(TAG,"Publisher_Task");
    char *root_topic = "LSE/blanca.nadal@alumnos.upm.es/workers";
